...
export default function Crossword() {
  ...
  useEffect(() => {
    ...
    getCompletedCrosswords(auth.currentUser, "mini").then(
      (completed: string[]) => {
        if (completed.includes(simpleHash)) {
          ...
          updateCompletedCrosswords(auth.currentUser, simpleHash, "mini");
        }
      },
    );

    getHighScore(auth.currentUser, "mini").then((score) => {
      if (
        !score ||
        ...
        setHighScore(auth.currentUser, stoppedTime, formattedDate, "mini");
  ...
  const updateCrossword = () => {
    ...
    saveCrossword(jsonCrosswordString, "mini");
              ...
              <div key={y} className="flex">
                {row.map((box: CrossWordBoxData, x) => (
                  <div
                    ...
                    className={`w-[120px] h-[120px] max-md:w-[35px] max-md:h-[35px] max-sm:w-[25px] max-sm:h-[25px] max-xs:w-[20px] max-xs:h-[20px] border-[0.5px] border-secondary-900 cursor-pointer flex items-center justify-center relative 
